...
async function diaUtil(dataStr) {
  // fim de semana
...
app.get('/available', async (req, res) => {
  ...
  if (!diaUtil(date)) return res.json({ date, disponiveis: [] });
...
app.post('/appointments', async (req, res) => {
  ...
  if (!diaUtil(date)) return res.status(400).json({ erro: 'data indisponível' });
  ...
  res.status(201).json({ id: info.lastInsertRowid, date, time, name });
});
